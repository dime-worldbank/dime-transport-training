...
library(basemaps)  # Basemaps on static maps
```
...
Lets add a basemap; we'll use the "basemap_gglayer" from the "basemaps" package

```{r, out.width = "35%"}
basemap_ggplot(city_sf)
```
...
`basemap_gglayer` uses the Web Mercator CRS (EPSG: 3857); we need to transform our other datasets to that CRS.

```{r, out.width = "35%", message = FALSE, warning = FALSE}
schools_sf_web <- schools_sf %>% st_transform(3857)

basemap_ggplot(city_sf) +
  geom_sf(data = schools_sf_web,
          aes(color = "Schools")) +
  labs(fill = "Area",
       color = NULL) +
  scale_color_manual(values = "black") +
